...
#SBATCH --output=/mnt/lustre/work/bamler/zxiao29/haolong/glue-factory/log/homography_reproduce-%j.out       # File to which STDOUT will be written - make sure this is not on $HOME
#SBATCH --error=/mnt/lustre/work/bamler/zxiao29/haolong/glue-factory/log/homography_reproduce-%j.err        # File to which STDERR will be written - make sure this is not on $HOME
...
git add -u
git commit -m "homography_reproduce"

source ~/.bashrc
srun /mnt/lustre/work/bamler/zxiao29/.miniconda3/envs/haolong/bin/python -m gluefactory.train homography_reproduce --conf gluefactory/configs/superpoint+simpleglue_homography.yaml  \
